...
for k in array:
    ...
    p1.savefig('C:/Users/Administrator/Desktop/项目数据/可视化/2017年%s售货机全年饮料销售额折线图.png' % (k))
    plot_acf(values, title='2017年%s售货机全年饮料销售额自相关图'%(k))
    plt.savefig('C:/Users/Administrator/Desktop/项目数据/可视化/2017年%s售货机全年饮料销售额自相关图.png' % (k))
    print("%s售货机原始ADF检验："%(k))
    print(sm.tsa.stattools.adfuller(values))
    # values1 = values.diff()[1:]
    # print("%s售货机一阶差分后："%(k))
    # print(sm.tsa.stattools.adfuller(values1))
    plot_acf(values, title='2017年%s售货机全年销售额一阶差分后自相关图' % (k))
    plt.savefig('C:/Users/Administrator/Desktop/项目数据/可视化/2017年%s售货机全年饮料销售额一阶差分后自相关图.png' % (k))
    print('BIC求解的模型阶次为', stattools.arma_order_select_ic(values,max_ma=3))